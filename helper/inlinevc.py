...
from jmbot.core import check_owner
...
# MAINMENU BUTTONS
@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^joinvc$")))
@check_owner(vc=True)
async def joinvc(event):
    ...
    try:
        vc_chat = await jmbot.get_entity(chat)
...
@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^leavevc$")))
@check_owner(vc=True)
...
@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^playervc$")))
@check_owner(vc=True)
...
# PLAYER BUTTONS
@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^menuvc$")))
@check_owner(vc=True)
async def playervc(event):
    await event.edit(file=vcimg, text="**| VC Menu |**", buttons=buttons[0])


@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^previousvc$")))
@check_owner
...
@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^resumevc$")))
@check_owner(vc=True)
...
@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^pausevc$")))
@check_owner(vc=True)
...
@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^skipvc$")))
@check_owner(vc=True)
...
@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^repeatvc$")))
@check_owner(vc=True)
...
@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^settingvc$")))
@check_owner
...
@jmbot.tgbot.on(CallbackQuery(pattern="^(a|b|c)mode$"))
@check_owner
async def vc(event):
...
@jmbot.tgbot.on(CallbackQuery(pattern="(a|b|c)modeinfo$"))
@check_owner(vc=True)
async def vc(event):
    mode = (event.pattern_match.group(1)).decode("UTF-8")
    if mode == "a":
        text = "⁉️ What is This?\n\n🏢 Public: Anyone can use catjmbot vc player present in this group.\n\n🏠 Private: Only Owner of user bot and sudo users can use catjmbot vc player"
    elif mode == "b":
...
# COMMON BUTTONS
@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^backvc$")))
@check_owner(vc=True)
async def vc(event):
    await event.edit("** | VC Menu | **", buttons=buttons[0])


@jmbot.tgbot.on(CallbackQuery(data=re.compile(r"^vc_close$")))
@check_owner(vc=True)
async def vc(event):
    try:
        await event.delete()
