...
from jmbot import Config, jmbot
from jmbot.core.managers import edit_or_reply
from jmbot.helpers import _catutils

from .vcp_helper import jmbotVC
...
async def handler(_, update):
    ...
    if vc_player.BOTMODE:
        if resp and type(resp) is list:
            ...
            event = await jmbot.tgbot.send_file(
                vc_player.CHAT_ID, file=resp[0], caption=caption, buttons=buttons
            )
        elif resp and type(resp) is str:
            resp = resp.split(f"\n\n")[1] if f"\n\n" in resp else resp
            event = await jmbot.tgbot.send_message(
                vc_player.CHAT_ID, resp, buttons=buttons
            )
    else:
        try:
            results = await jmbot.inline_query(Config.TG_BOT_USERNAME, "vcplayer")
            event = await results[0].click(vc_player.CHAT_ID, hide_via=True)
        except Exception:
            if resp and type(resp) is list:
                caption = resp[1].split(f"\n\n")[1] if f"\n\n" in resp[1] else resp[1]
                event = await jmbot.send_file(
                    vc_player.CHAT_ID, file=resp[0], caption=caption, buttons=buttons
                )
            elif resp and type(resp) is str:
                resp = resp.split(f"\n\n")[1] if f"\n\n" in resp else resp
                event = await jmbot.send_message(
                    vc_player.CHAT_ID, resp, buttons=buttons
                )
...
async def check_vcassis(event):
    chat = await jmbot.get_entity(event.chat_id)
    participants = await jmbot.get_participants(chat)
    assis = await vc_player.client.get_me()
    cat_ub = await jmbot.get_me()
    get_id = assis.id
...
async def vc_reply(event, text, file=False, firstmsg=False, dlt=False, **kwargs):
    me = await jmbot.get_me()
    if vc_player.BOTMODE:
        try:
            if file:
                catevent = await jmbot.tgbot.send_file(
                    event.chat_id, file=file, caption=text, **kwargs
                )
            else:
                catevent = (
                    await jmbot.tgbot.send_message(event.chat_id, text, **kwargs)
                    if firstmsg
        ...
        try:
            ...
            catevent = await jmbot.send_file(
                event.chat_id, file=file, caption=text, **kwargs
            )
    elif vc_player.PUBLICMODE:
        catevent = (
            await jmbot.send_message(event.chat_id, text, **kwargs)
