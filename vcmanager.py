...
from jmbot.core.data import _sudousers_list, _vcusers_list
from jmbot.core.managers import edit_delete, edit_or_reply
from jmbot.helpers.utils import mentionuser
from jmbot.sql_helper import global_collectionjson as sql
...
async def get_group_call(chat):
    if isinstance(chat, Channel):
        result = await jmbot(functions.channels.GetFullChannelRequest(channel=chat))
    elif isinstance(chat, Chat):
        result = await jmbot(functions.messages.GetFullChatRequest(chat_id=chat.id))
...
async def parse_entity(entity):
    if entity.isnumeric():
        entity = int(entity)
    return await jmbot.get_entity(entity)
...
async def start_vc(event):
    ...
    vc_chat = await jmbot.get_entity(event.chat_id)
...
async def end_vc(event):
    ...
    if not gc_call:
        return
    try:
        await jmbot(functions.phone.DiscardGroupCallRequest(call=gc_call))
...
async def title_vc(event):
    ...
    await jmbot(functions.phone.EditGroupCallTitleRequest(call=gc_call, title=title))
    ...
    if cmd == "get":
        if not vc_chats:
            return await edit_delete(
                event, "__There are no vc auth users for your Catjmbot.__"
            )
        result = "**The list of vc auth users for your Catjmbot are :**\n\n"
        for chat in [*vcusers]:
        ...
        if replied_user is None:
            return
        replied_user = await jmbot.get_entity(replied_user)
